...
class Runner():
    ...
    - Make a mini batch
    """
    def __init__(self, env_cfg, model_cfg, nsteps, state_shape, action_size):
        ...
        self.agent = DoubleDQNAgent(state_shape, action_size, network=model_cfg['network'],
    ...
    def run(self, params):
        ...
        for _ in range(self.nsteps):
            ...
            #state = self.obs
            #print(state)
            # print("ball", self.obs[0].keys())
            # print("ball", self.obs[0]["ball"])
            dist = dist2gate(self.obs[0]["ball"][0:2])
            action = self.agent.get_action(state)
            self.obs, reward, done, _ = self.env.step(action)
            next_dist = dist2gate(self.obs[0]["ball"][0:2])
            if reward == 0.0:
                reward = dist - next_dist
                reward = seg_reward(reward)
            #print("reward", reward)
...
def learn(*, address, nenvs, network="mlp", env_cfg, total_timesteps, state_shape=[72,96,4], nsteps=16,
    ...
    agent = DoubleDQNAgent(state_shape, action_size, network=network,
    ...
    for i in range(nenvs):
        runner = Runner.remote(env_cfg=env_cfg, model_cfg=model_cfg, nsteps=nsteps, state_shape=state_shape, action_size=action_size)
